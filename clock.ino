...
#define CLOCK2_X 0           // позиция часов по X (начало координат - левый нижний угол)
#define CLOCK2_Y 9           // позиция часов по Y (начало координат - левый нижний угол)
#define COLOR_MODE 1        // Режим цвета часов
...
void drawClock2(byte day, boolean dots, byte X, byte Y) {
  drawDigit3x5(day / 10, X + 4, Y, clockLED[6]);
  drawDigit3x5(day % 10, X + 8, Y, clockLED[5]);
}
...
void clockRoutine() {
  ...
  if (!clockSet) {
    ...
    drawClock2(day, dotFlag, CLOCK2_X, CLOCK2_Y);
  } else {
    ...
    drawClock2(day, 1, CLOCK2_X, CLOCK2_Y);
  }
}
...
void clockOverlayWrap(byte posX, byte posY) {
  ...
  drawClock2(day, dotFlag, posX, posY);
}
